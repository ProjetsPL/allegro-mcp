...
import {
  Billing,
  BillingListBillingEntriesParams,
  BillingListBillingEntriesResponse,
  BillingListBillingTypesParams,
  BillingListBillingTypesResponse,
} from './resources/billing';
import {
  Charity,
  CharityListFundraisingCampaignsParams,
  CharityListFundraisingCampaignsResponse,
} from './resources/charity';
import {
  MarketplaceItemCurrency,
  MarketplaceItemLanguage,
  MarketplaceListResponse,
  Marketplaces,
} from './resources/marketplaces';
import { Me, MeRetrieveResponse } from './resources/me';
import {
  ChangePriceInput,
  ListingOffer,
  ListingResponseFilters,
  OfferChangePriceParams,
  OfferChangePriceResponse,
  OfferListParams,
  OfferListResponse,
  Offers,
...
import {
  Coordinates,
  PointsOfService,
  PointsOfServiceCreateParams,
  PointsOfServiceListParams,
  PointsOfServiceListResponse,
  PointsOfServiceUpdateParams,
  Pos,
  Seller,
} from './resources/points-of-service';
import {
  Pricing,
  PricingCalculateFeePreviewParams,
  PricingCalculateFeePreviewResponse,
  PricingGetOfferQuotesParams,
  PricingGetOfferQuotesResponse,
} from './resources/pricing';
import { UserRetrieveRatingsSummaryResponse, Users } from './resources/users';
import { readEnv } from './internal/utils/env';
import { formatRequestDetails, loggerFor } from './internal/utils/log';
import { isEmptyObj } from './internal/utils/values';
import { Account } from './resources/account/account';
import { AfterSalesServiceConditions } from './resources/after-sales-service-conditions/after-sales-service-conditions';
import { Bidding } from './resources/bidding/bidding';
import {
  Fulfillment,
  FulfillmentGetAvailableProductsParams,
  FulfillmentGetAvailableProductsResponse,
  FulfillmentGetStockParams,
  FulfillmentGetStockResponse,
} from './resources/fulfillment/fulfillment';
import { Messaging } from './resources/messaging/messaging';
import {
  Order,
  OrderGetEventStatsResponse,
  OrderListEventsParams,
  OrderListEventsResponse,
} from './resources/order/order';
import {
  PaymentListOperationsParams,
  PaymentListOperationsResponse,
  Payments,
...
import {
  ...
  ClassifiedDailyEventStat,
  ClassifiedEventStat,
  MarketplaceReference,
  ProductCategory,
  ProductParameter,
  Sale,
  SaleGetAllegroPricesAccountEligibilityResponse,
  SaleGetBadgeCampaignsParams,
  SaleGetBadgeCampaignsResponse,
  SaleGetBadgeOperationDetailsResponse,
  SaleGetCategoryEventsParams,
  SaleGetCategoryEventsResponse,
  SaleGetCategoryParametersScheduledChangesParams,
  SaleGetCategoryParametersScheduledChangesResponse,
  SaleGetClassifiedOffersStatsParams,
  SaleGetClassifiedOffersStatsResponse,
  SaleGetClassifiedSellerStatsParams,
  SaleGetClassifiedSellerStatsResponse,
  SaleGetCompatibilityListSuggestionsParams,
  SaleGetCompatibilityListSuggestionsResponse,
  SaleGetDeliveryMethodsParams,
  SaleGetDeliveryMethodsResponse,
  SaleGetMatchingCategoriesParams,
  SaleGetMatchingCategoriesResponse,
  SaleGetOfferEventsParams,
  SaleGetOfferEventsResponse,
  SaleGetOfferPromotionPackagesResponse,
  SaleGetQualityResponse,
  SaleGetSizeTablesTemplatesResponse,
  SaleGetSmartSellerClassificationReportParams,
  SaleGetSmartSellerClassificationReportResponse,
  SaleGetTaxSettingsParams,
  SaleGetTaxSettingsResponse,
  SaleProposeProductParams,
  SaleProposeProductResponse,
  SaleUpdateAllegroPricesAccountConsentParams,
  SaleUpdateAllegroPricesAccountConsentResponse,
  SaleUploadImageParams,
  SaleUploadImageResponse,
} from './resources/sale/sale';
import {
  PickupAddress,
  ShipmentManagement,
  ShipmentManagementCreateLabelParams,
  ShipmentManagementCreateProtocolParams,
  ShipmentManagementListDeliveryServicesResponse,
  ShipmentManagementListPickupProposalsParams,
  ShipmentManagementListPickupProposalsResponse,
} from './resources/shipment-management/shipment-management';

export interface ClientOptions {
  /**
   * Defaults to process.env['ALLEGRO_API_API_KEY'].
   */
  apiKey?: string | null | undefined;

  /**
   * Defaults to process.env['ALLEGRO_API_ENVIRONMENT'].
   */
   ...
   *
   * Defaults to process.env['ALLEGRO_API_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['ALLEGRO_API_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Allegro API API.
 */
export class AllegroAPI {
   ...
   * API Client for interfacing with the Allegro API API.
   *
   * @param {string | null | undefined} [opts.apiKey=process.env['ALLEGRO_API_API_KEY'] ?? null]
   * @param {string | undefined} [opts.environment=process.env['ALLEGRO_API_ENVIRONMENT'] ?? allegro.pl]
   * @param {string} [opts.baseURL=process.env['ALLEGRO_API_BASE_URL'] ?? https://api.{environment}] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('ALLEGRO_API_BASE_URL'),
    apiKey = readEnv('ALLEGRO_API_API_KEY') ?? null,
    environment = readEnv('ALLEGRO_API_ENVIRONMENT') ?? 'allegro.pl',
    ...opts
  }: ClientOptions = {}) {
    const options: ClientOptions = {
      ...
      environment,
      ...opts,
      baseURL: baseURL || `https://api.${environment}`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? AllegroAPI.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('ALLEGRO_API_LOG'), "process.env['ALLEGRO_API_LOG']", this) ??
      defaultLogLevel;
  ...
  static AllegroAPI = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static AllegroAPIError = Errors.AllegroAPIError;
  ...
  sale: API.Sale = new API.Sale(this);
  offers: API.Offers = new API.Offers(this);
  users: API.Users = new API.Users(this);
  me: API.Me = new API.Me(this);
  pricing: API.Pricing = new API.Pricing(this);
  pointsOfService: API.PointsOfService = new API.PointsOfService(this);
  afterSalesServiceConditions: API.AfterSalesServiceConditions = new API.AfterSalesServiceConditions(this);
  order: API.Order = new API.Order(this);
  account: API.Account = new API.Account(this);
  payments: API.Payments = new API.Payments(this);
  bidding: API.Bidding = new API.Bidding(this);
  billing: API.Billing = new API.Billing(this);
  charity: API.Charity = new API.Charity(this);
  shipmentManagement: API.ShipmentManagement = new API.ShipmentManagement(this);
  marketplaces: API.Marketplaces = new API.Marketplaces(this);
  messaging: API.Messaging = new API.Messaging(this);
  fulfillment: API.Fulfillment = new API.Fulfillment(this);
}
AllegroAPI.Sale = Sale;
AllegroAPI.Offers = Offers;
AllegroAPI.Users = Users;
AllegroAPI.Me = Me;
AllegroAPI.Pricing = Pricing;
AllegroAPI.PointsOfService = PointsOfService;
AllegroAPI.AfterSalesServiceConditions = AfterSalesServiceConditions;
AllegroAPI.Order = Order;
AllegroAPI.Account = Account;
AllegroAPI.Payments = Payments;
AllegroAPI.Bidding = Bidding;
AllegroAPI.Billing = Billing;
AllegroAPI.Charity = Charity;
AllegroAPI.ShipmentManagement = ShipmentManagement;
AllegroAPI.Marketplaces = Marketplaces;
AllegroAPI.Messaging = Messaging;
AllegroAPI.Fulfillment = Fulfillment;
export declare namespace AllegroAPI {
  ...
  export { Users as Users, type UserRetrieveRatingsSummaryResponse as UserRetrieveRatingsSummaryResponse };
  ...
  export {
    PointsOfService as PointsOfService,
  ...
  export { AfterSalesServiceConditions as AfterSalesServiceConditions };
  ...
  export {
    ShipmentManagement as ShipmentManagement,
