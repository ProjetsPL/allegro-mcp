...
import { ClientOptions } from 'allegro-api';
...
export { ClientOptions } from 'allegro-api';
export { endpoints } from './tools';

// Create server instance
export const server = new McpServer(
  {
    name: 'allegro_api_api',
    version: '1.1.0',
  },
  {
    capabilities: {
...
export function initMcpServer(params: {
  ...
  const client = new AllegroAPI(params.clientOptions);
  const capabilities = {
    ...defaultClientCapabilities,
...
export function init(params: {
  ...
  const client = params.client || new AllegroAPI({ defaultHeaders: { 'X-Stainless-MCP': 'true' } });
